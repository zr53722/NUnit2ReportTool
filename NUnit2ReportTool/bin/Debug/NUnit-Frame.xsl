...
<xsl:template name="all.packages">
    <html>
		...
		<h2><a href="overview-summary.html"  id=":i18n:Home" target="classFrame">Home</a></h2>
            ...
            <h2 id=":i18n:Assemblies">Assemblies</h2>
			<!-- //test-results/@name -->
            <table width="100%">
				<tr>
					<td nowrap="nowrap">
...
<xsl:template name="overview.packages">
    <html>
		...
		<xsl:variable name="totalCount" select="@total"/><!-- testCount -->
		...
		<xsl:variable name="runCount" select="$totalCount - $failureCount"/>
			...
			<td  width="280px">
				...
				<xsl:if test="round($runCount * 200 div $total )!=0">
					<span class="covered">
						...
						<center><xsl:value-of select="$runCount"/></center>
					</span>
				</xsl:if>
				...
				<span class="ignored">
					...
					<center><xsl:value-of select="$ignoreCount"/></center>
				</span>
				</xsl:if>
				<xsl:if test="round($failureCount * 200 div $total )!=0">
					<span class="uncovered">
						...
						<center><xsl:value-of select="$failureCount"/></center>
					</span>
				</xsl:if>
				<span class="total">
		...
		<span id=":i18n:Note">Note</span>: <i id=":i18n:errors">errors</i>&#160;<span id=":i18n:anticipated">are anticipated and checked for with assertions while</span>&#160;&#160;<i id=":i18n:failures">failures</i>&#160;<span id=":i18n:unanticipated">are unanticipated.</span>
		...
		<table border="0" cellpadding="2" cellspacing="0" width="95%">
			...
			<xsl:for-each select="//test-suite[(child::results/test-case)]">
				...
				<xsl:variable name="errorCount2" select="count(child::results/test-case[@executed='False'])"/>
				<xsl:variable name="failureCount2" select="count(child::results/test-case[@success='False'])"/>
				<xsl:variable name="runCount2" select="$testCount2 - $errorCount2 - $failureCount2"/>
				<xsl:variable name="timeCount2" select="translate(@time,',','.')"/>
					...
					<td width="40%" height="9px">
						<xsl:if test="round($runCount2 * 200 div $testCount2 )!=0">
							<span class="covered">
								...
								<center><xsl:value-of select="$runCount2"/></center>
							</span>
						</xsl:if>
						...
						<span class="ignored">
							...
							<center><xsl:value-of select="$errorCount2"/></center>
						</span>
						</xsl:if>
						<xsl:if test="round($failureCount2 * 200 div $testCount2 )!=0">
							<span class="uncovered">
								...
								<center><xsl:value-of select="$failureCount2"/></center>
							</span>
						</xsl:if>
						<span class="total">
							&#160;<xsl:value-of select="$testCount2"/>
						</span>
					</td>
<!--  ******************************************************* -->
...
<xsl:template name="all.classes" >
    <html>
        ...
        <body>
            <h2 id=":i18n:TestSuite">TestSuite</h2>
...
<xsl:template name="test-case">
    ...
    <html>
        ...
        <body>
			...
			<h3 id=":i18n:TestSuite">TestSuite</h3>
...
<!-- ******************************************************* -->
					...
					<td width="40%" height="9px">
						...
						<span class="ignored">
							...
							<center><xsl:value-of select="$errorCount"/></center>
						</span>
						</xsl:if>
...
<!--  ******************************************************* -->
			...
			<br/>
			<h3 id=":i18n:TestCase">Test Case</h3>
